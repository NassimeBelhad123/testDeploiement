        ...
        const recupererStageIdPostuler = async() => {
            try {
                const reponseData = await sendRequest(
                    process.env.REACT_APP_BACKEND_URL+ `etudiants/recupererStagePostulerId/${etudiantId}`
                );
    ...
    useEffect(() => {
        const recupererStage = async () => {
            ...
            for(let i = 0; i < tableauStageIdPostuler.length; i++) {
                try{
                    const reponseData = await sendRequest(
                        process.env.REACT_APP_BACKEND_URL+ `stages/recupererStage/${tableauStageIdPostuler[i].stage}`
                    );
