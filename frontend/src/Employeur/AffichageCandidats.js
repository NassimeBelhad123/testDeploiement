    ...
    const recupererEtudiantsDetails = async () => {
      const details = await Promise.all(
        tableauEtudiant.map(async (etudiantId) => {
          try {
            const reponseData = await sendRequest(
              process.env.REACT_APP_BACKEND_URL+`etudiants/recupererEtudiant/${etudiantId}`
            );
  ...
  const submitDecision = async (selectedValue, etudiantId) => {
    try {
        const response = await sendRequest(
          process.env.REACT_APP_BACKEND_URL+ `etudiants/modifierPostulationReponse/${etudiantId}/${stageId}`,
            'PATCH',
            JSON.stringify({ 
